...
public class QDoxUtil {
  ...
  public static List<String> getDiscoverableClassNames(String packageName, String konfigurable) {
    ...
    // list to return
    List<String> discoverableClasses = new ArrayList<>();
...
//        }
   ...
   */
  public static List<KonfigurableMetaData> generateModel(String packageName, List<String> discoverableClasses) {
      ...
      do {
        ...
        for (JavaField javaField : cls.getFields()) {
          ...
          for (JavaAnnotation javaAnnotation : fieldAnnotations) {
            if (javaAnnotation.getType().getCanonicalName().equals("com.fasterxml.jackson.annotation.JsonProperty")) {
              isJsonProperty = true;
              break;
            }
          }
          if (isJsonProperty) {
            ...
            // set it's desciption if it has one
            ...
            for (JavaAnnotation javaAnnotation : fieldAnnotations) {
              ...
              if (javaAnnotation.getType().getCanonicalName().equals("javax.validation.constraints.NotNull")) {
                konfigurableProperty.setRequired(true);
              }
              if (javaAnnotation.getType().getCanonicalName().equals("javax.validation.constraints.Min")) {
                konfigurableProperty.setMinValue(javaAnnotation.getProperty("value").toString());
              }
              if (javaAnnotation.getType().getCanonicalName().equals("javax.validation.constraints.Max")) {
          ...
          // System.out.println("---" + javaField.getName() + " : " + Arrays.toString(javaField.getAnnotations().toArray()));
